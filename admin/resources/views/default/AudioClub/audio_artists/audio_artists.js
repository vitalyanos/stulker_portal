        ...
        function TestTable1() {
            $('#datatable-1').on('xhr.dt', function (e, settings, json) {
                    ...
                    for (var i in json.data) {
                        ...
                        json.data[i].operations = "<div class='col-xs-3 col-sm-8'>\n\
                                                        ...
                                                        <ul class='dropdown-menu pull-right'>\n\
                                                            <li>\n\
                                                                <a class='no_context_menu' href='{{ app.request_context.baseUrl }}/{{ app.controller_alias }}/edit-audio-artists' data-artistsid='" + id + "' id='edit_audio_artists_" + id + "'>\n\
                                                                    <span>{{ 'Edit'|trans }}</span>\n\
                                                                </a>\n\
                                                            </li>\n\
                                                            <li>\n\
                                                                <a " + (albums_count != "0" ? "disabled='disabled' warning_msg='{{ 'First, remove all the albums of this performer'|trans }}'" : "") + " href='#' class='remove-audio-artists no_context_menu' data-artistsid='" + id + "'>\n\
                        ...
                        json.data[i].name = '<a class="no_context_menu" href="{{ app.request_context.baseUrl }}/{{ app.controller_alias }}/edit-audio-artists" data-artistsid="' + id + '" id="edit_audio_artists_' + id + '">' + name + '</a>';
                    }
                }
            }).dataTable({
                "processing": true,
                "serverSide": true,
                "ajax": {
                    "url": "{{ app.request_context.baseUrl }}/{{ app.controller_alias }}/audio-artists-list-json"
                },
                "language": {
        ...
        function yelp() {
            $(document).ready(function () {
                ...
                $(document).on('click submit', "#modalbox button[type='submit'], #modalbox form", function (e) {
					...
					if( !$('#audio_artists_name').val() ) {
                        $('#audio_artists_name').css('border','1px solid red');
					} else {
                        $('#audio_artists_name').css('border','');
                        var sendData = {
...
{# this templates moved to .twig file and left for demonstrate purpose only 
    ...
    <script type="text/template" id="modal_del_body">
        <div class="col-md-12">
            <span class="col-md-12 txt-default">{{ 'Do you really want delete this performer?'|trans }}</span>
        </div>
    </script>

    <script type="text/template" id="modal_del_buttons">
        <div class="pull-right no-padding">
            <a id="del_artist_btn" type="button" class="main_ajax btn btn-danger pull-right no_context_menu" href='{{ app.request_context.baseUrl }}/{{ app.controller_alias }}/remove-audio-artists'>{{ 'Delete'|trans }}</a>
            <button type="reset" id="del_artist_button" class="btn btn-success pull-right">{{ 'Cancel'|trans }}</button>
        </div>
    </script>
